...
#General precondition
	...
	#Change the initials variable below to match the initials of the student
	#	run being performed
...
#Code file requirements
	...
	#Code file should be a valid cpp file and should
	#	 not require any other files to compile

#Test file requirements
	#Test files should be kept in a subfolder named 
	#	unit_tests under where this script is kept
	#Test files should be named mirrorIn01.txt and 
	#	mirrorOut01.txt with a different number for each pair
	#Test files with the same number should have a 
	#	matching output for the given input file.
	#The test_count section should be 
	#	updated to reflect the number of test cases
	#Test case files should be numbered 
	#	with two digits from 01 to test_cases
	#Test files must have a blank line at the 
	#	end to ensure that the compare operation reads the whole file
...
#Output of this script
	...
	#At the end it will either say 
	#	"All test cases passed" or "N test cases FAILED"
	#N in the failure message will reflect the number of cases that failed

#Coverage test output
   #In addition to outputing the test 
   #	run information, coverage test data is created.
   #Results of the coverage test can be found in mirrorCoverageTestRsltsBTL.txt
...
g++ -fprofile-arcs -ftest-coverage -o mirror$initials mirror$initials.o
rm mirror$initials.o
rm $out_file
rm mirrorCoverageTestRsltsBTL.txt
...
gcov mirror$initials.cpp
cat mirror$initials.cpp.gcov > mirrorCoverageTestRsltsBTL.txt
